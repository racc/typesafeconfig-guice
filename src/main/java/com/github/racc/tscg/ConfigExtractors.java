...
enum ConfigExtractors implements ConfigExtractor {
	...
	static Optional<Object> extractConfigValue(Config config, Class<?> paramClass, Type paramType, String path) {
		if (EXTRACTOR_MAP.containsKey(paramClass)) {
			return Optional.of(EXTRACTOR_MAP.get(paramClass).extractValue(config, paramType, path));
		} else {
